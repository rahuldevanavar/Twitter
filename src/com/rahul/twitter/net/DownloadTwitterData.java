...
import com.rahul.twitter.comman.TwitItems;
...
public class DownloadTwitterData extends AsyncTask<String, Void, String> {
	...
	protected void onPostExecute(String result) {
		ArrayList<TwitItems> items;
		items = getParsedData(result);
		mListener.onResultComplete(items);
	}

	private ArrayList<TwitItems> getParsedData(String result) {
		ArrayList<TwitItems> items = new ArrayList<TwitItems>();
		try {
			...
			for (int i = 0; i < jsonArray.length(); i++) {
				...
				TwitItems tweetItems = new TwitItems();
	...
	public interface OnResultListener {
		void onResultComplete(ArrayList<TwitItems> items);
	}
}